...
export default function CompaniesPage() {
  ...
  const query = useQuery({
    ...
    queryFn: () =>
      apiList<Company>(
        `/companies?limit=100&search=${encodeURIComponent(search)}`,
      ),
  });
  return (
    <>
      <PageHeader
        title="Companies"
        description="Workspace-scoped organizations and agency partners."
        action={
            ...
            <Table>
              <TableHeader>
                <TableRow>
                  ...
                  <TableHead>Partnership fit</TableHead>
                  ...
                  <TableRow key={company.id}>
                    <TableCell>
                      ...
                      <div className="text-xs text-slate-500">
                        {company.domain ?? "No domain"} · discovered{" "}
                        {company.discoveryCount ?? 0}x
                      </div>
                    </TableCell>
                    <TableCell>
                    ...
                    <TableCell>{company.employeeRange ?? "—"}</TableCell>
                    <TableCell>
                      <span className="font-semibold">
                        {company.partnershipFitScore}
                      </span>
                      /100
                    </TableCell>
                    <TableCell>
                      <div className="flex justify-end gap-1">
                        <FindContacts company={company} />
                        <Button
                          variant="ghost"
                          size="icon"
...
function CompanyModal({
  ...
  const form = useForm<FormValues>({
    ...
    values: {
      ...
      companyType: value?.companyType ?? "AGENCY",
    },
  });
  const mutation = useMutation({
    mutationFn: (data: FormValues) =>
      apiFetch<Company>(value ? `/companies/${value.id}` : "/companies", {
  ...
  return (
    <Modal
      ...
      onClose={onClose}
      title={value ? "Edit company" : "Add company"}
      description="Core company data is stored by the independent API."
    >
      <form
      ...
      >
        ...
        <Field label="Employee range">
          <Input placeholder="11-50" {...form.register("employeeRange")} />
        </Field>
        <Field label="Company type">
          <Input placeholder="AGENCY" {...form.register("companyType")} />
        </Field>
        {mutation.error && (
...
function FindContacts({ company }: { company: Company }) {
  const client = useQueryClient();
  const mutation = useMutation({
    mutationFn: () =>
      apiFetch(`/companies/${company.id}/find-contacts`, { method: "POST" }),
    onSuccess: async () => {
      await Promise.all([
...
function DeleteCompany({ company }: { company: Company }) {
  ...
  const mutation = useMutation({
    mutationFn: () =>
      apiFetch(`/companies/${company.id}`, { method: "DELETE" }),
  ...
  return (
    <Button
      ...
      onClick={() => {
        if (
          window.confirm(
            `Delete ${company.name}? Contacts and leads will be unassigned.`,
          )
        )
          mutation.mutate();
      }}
