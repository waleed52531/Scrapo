...
export class AuthGuard implements CanActivate {
  ...
  async canActivate(context: ExecutionContext) {
    ...
    const workspace = await this.workspaceContext.resolve(
      identity,
  ...
  private async verifySupabaseToken(token: string) {
    ...
    try {
      ...
      if (header.alg?.startsWith("HS") && !legacySecret)
        throw new Error("Legacy JWT secret is not configured.");
