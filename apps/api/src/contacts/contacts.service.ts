...
export class ContactsService {
    ...
    try {
      return await this.prisma.$transaction(async (tx) => {
        const contact = await tx.contact.create({
          data: { workspaceId, ...input, email: input.email?.toLowerCase() },
        });
  ...
  async verifyEmail(workspaceId: string, userId: string, id: string) {
    const contact = await this.get(workspaceId, id);
    if (!contact.email) {
      throw new NotFoundException({
        code: "CONTACT_EMAIL_NOT_FOUND",
        message: "This contact does not have an email to verify.",
      });
    }
    await this.prisma.auditLog.create({
      data: {
        workspaceId,
        actorUserId: userId,
        action: "CONTACT_EMAIL_VERIFICATION_QUEUED",
        entityType: "Contact",
        entityId: id,
      },
    });
    return this.analysisQueue.enqueue(workspaceId, "VERIFY_EMAIL", {
      contactId: id,
    });
  }
